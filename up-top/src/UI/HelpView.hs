...
drawHelp st = 
  [ center $ padLeftRight 2 $ 
    hLimit 50 $ 
    vBox [ str $ "uptop " <> st ^. version ^. versionNumber <> " - (C) 2020 Leo Orpilla III"
         , vLimit 1 $ fill ' '
         , drawControls
         , vLimit 1 $ fill ' '
         , drawVimControls
...
drawControls =
  vBox [ hBorderWithLabel (str "Controls")
       , vLimit 1 $ fill ' '
...
drawVimControls = 
  vBox [ hBorderWithLabel (str "Vim-style")
       , vLimit 1 $ fill ' '
